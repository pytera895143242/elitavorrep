...
async def answer_push_inline_button(call, state: FSMContext):
    ...
    if call.data == 'go_3':
        ...
        await bot.copy_message(from_chat_id=content, chat_id=call.message.chat.id, message_id=11)
        await asyncio.sleep(2)
        await bot.copy_message(from_chat_id=content, chat_id=call.message.chat.id, message_id=17)
    ...
    if call.data == 'go_4':
        markup = types.InlineKeyboardMarkup()
        bat_a = types.InlineKeyboardButton(text='Let’s go 💵', url='https://t.me/BekirSPRINTbot?start=SprintArbitrage')
        markup.add(bat_a)
